...
\begin{document}
    ...
    \begin{enumerate}
        ...
        %
        \boxans{
            Théoriquement, le spectre de $V\p{t}$ peut être infini, alors que la fréquence $f_\text e$ est forcément finie, et ainsi il y forcément un chevauchement des copies du spectre de $V\p{t}$, et donc une perte d'information. En pratique cependant, on peut considérer que l'information passée une fréquence $f_\text{max}$, est négligeable. Ainsi, il s'agit de s'assurer que $f_\text{max} < f_\text e - f_\text{max}$, c'est-à-dire que $f_\text e > 2f_\text{max}$. 
        }
        %
        \yesafter
        %
