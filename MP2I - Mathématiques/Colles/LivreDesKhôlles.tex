...
\begin{document}
    ...
    \begin{enumerate}
        \item Montrer que $\sh$ admet une bijection réciproque $\argsh$ définie sur $\bdR$.
        
        En donner une expression explicite, et une expression explicite de la dérivée.
    
        \boxans{
            La fonction $\sh$ est strictement croissante et continue sur $\bdR$, et de plus :
            %
            \[ \lim\limits_{x \to -\infty} \sh x = -\infty \qquad\et\qquad \lim\limits_{x \to +\infty} \sh x = +\infty \]
            %
            Ainsi, en vertu du \textit{théorème de la bijection}, $\sh$ est une bijection de $\bdR$ dans $\bdR$ et admet admet une bijection réciproque, strictement croissante et continue de $\bdR$ et $\bdR$, qu'on nommera $\argsh$.\medskip
        
            Soient deux réels $\p{x, y} \in \bdR^2$ tels que $\sh x = y$, c'est-à-dire tels que $x = \argsh y$. On a :
            %
            \[ \sh x = y \qquad\text{donc}\qquad \dfrac{e^x-e^{-x}}{2} = y \qquad\text{donc}\qquad \p{e^x}^2 - 2ye^x - 1 = 0\]
            %
            On pose alors $X = e^x$, de sorte que $X^2 - 2yX - 1 = 0$. On reconnaît un polynôme de degré $2$ en $X$, de discriminant $\Delta = 4y^2 + 4 = 4\p{y^2 + 1} > 0$. Les solutions de l'équation sont donc :
            %
            \[ X_\pm = \dfrac{2y \pm 2\sqrt{y^2 + 1}}{2} = y \pm \sqrt{y^2 + 1} \qquad\text{et par positivité de l'exponentielle} \ X > 0\ \text{donc}\qquad X = y + \sqrt{y^2 + 1}\]
            %
            On obtient finalement que $x = \ln{y + \sqrt{y^2 + 1}}$, donc pour tout réel $x$ on a $\argsh x = \ln{x + \sqrt{x^2 + 1}}$.
            
            Par opérations (somme, composition) $\argsh$ est dérivable sur $\bdR$. On obtient :
            %
            \[ \forall x \in \bdR,\qquad \argsh'\p{x} = \dfrac{1 + \frac{2x}{2\sqrt{x^2+1}}}{x+\sqrt{x^2+1}} = \dfrac{\frac{\sqrt{x^2+1}+x}{\sqrt{x^2+1}}}{x+\sqrt{x^2+1}} = \dfrac{1}{\sqrt{x^2+1}}\]
    }
    
    \item Montrer que $\th$ admet une bijection réciproque $\argth$ définie sur $\into{-1, 1}$.
    
    En donner une expression explicite, et une expression explicite de la dérivée.
    
    \boxans{
        La fonction $\th$ est strictement croissante et continue sur $\bdR$, et de plus :
        %
        \[ \lim\limits_{x \to -\infty} \th x = -1 \qquad\et\qquad \lim\limits_{x \to +\infty} \th x = 1 \]
        %
        Ainsi, en vertu du \textit{théorème de la bijection}, $\sh$ est une bijection de $\bdR$ dans $\into{-1, 1}$ et admet admet une bijection réciproque, strictement croissante et continue de $\into{-1, 1}$ et $\bdR$, qu'on nommera $\argth$.\medskip
        
        Soient deux réels $\p{x, y} \in \bdR^2$ et $y$ tels que $\th x = y$, c'est-à-dire tels que $\argth y = x$. On a :
        %
        \[ \th x = \dfrac{\sh x}{\ch x} = \dfrac{e^{2x}-1}{e^{2x}+1} = y \qquad\text{donc}\qquad e^{2x}-1 = y\left(e^{2x}+1\right) \qquad\text{donc}\qquad e^{2x}\left(1-y\right) = 1+y\]
        %
        On a donc $2x = \ln{\dfrac{1+y}{1-y}}$, donc pour tout réel $x$ on a $\argth x = \ln{\sqrt{\dfrac{1+x}{1-x}}}$.
        %
        Par opérations (somme, composition) $\argth$ est dérivable sur $\left] - 1, 1\right[$. On obtient :
        %
        \[ \forall x \in \bdR,\qquad \argth'\p{x} = \dfrac{1}{2\p{1+x}} - \dfrac{-1}{2\p{1-x}} = \dfrac{2}{2\p{1-x^2}} = \dfrac{1}{1-x^2} \]
        
    }
    \item Donner une primitive de $x \mapsto \dfrac{1}{\p{x -a}\p{x - b}}$ avec $\p{a, b} \in \bdR^2$ deux réels distincts.
    
    \boxans{
        Soit $f$ définie sur un intervalle $I \subset \bdR \backslash \{a, b\}$ par $f\p{x} =  \dfrac{1}{\p{x -a}\p{x - b}}$. On réduit $f$ en élément simples :
        %
        \[ \forall x \in I,\qquad f\p{x} = \dfrac{1}{\p{x - a}\p{x - b}} = \dfrac{1}{a-b}\p{\dfrac{1}{x-a}-\dfrac{1}{x-b}}\] 
        %
        Une primitive de $\dfrac{1}{x-a}$ est $\ln|x-a| + \text{cte}$. De même, une primitive de $\dfrac{1}{x-b}$ est $\ln|x-b| + \text{cte}$.
        
        Par linéarité, une primitive de $x \mapsto \dfrac{1}{\p{x -a}\p{x - b}}$ est donc $x \mapsto \dfrac{1}{a-b}\ln\mod{\dfrac{x-a}{x-b}}+ \text{cte}$.
    }
